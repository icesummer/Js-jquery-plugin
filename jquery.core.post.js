...
;(function(obj) {
	...
	_g.js = {
		...
		postAjax : function(url, par ,ext) {
			...
			ext=$.extend({},{type: "post",async:false,url:url,par:par,dataType:"json"},ext);
			var message = ({"msg":"操作异常！","code":2});
			if(ext.async){
				message = ({"msg":"执行中请稍后~","code":0});
			}
			$.ajax({
				type: ext.type,
				async:ext.async,
				...
				success: function(data){
					...
					message = ({"msg":"操作异常~"+JSON.stringify(e),"code":0});
					_g(postDom).remove();
				}
			});
			return message;
